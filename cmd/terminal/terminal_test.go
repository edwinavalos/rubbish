...
func TestSessionStartCmd_WithRepo(t *testing.T) {
	...
	want := `bash -l -c 'screen -D -R -S rubbish bash -l -c "cd /root/workspace/rubbish 2>/dev/null || cd ~; claude --dangerously-skip-permissions; exec bash -l"'`
	if got != want {
...
func TestSessionStartCmd_NoRepo(t *testing.T) {
	got := sessionStartCmd("")
	want := `bash -l -c 'screen -D -R -S rubbish bash -l -c "claude --dangerously-skip-permissions; exec bash -l"'`
	if got != want {
		t.Errorf("got %q, want %q", got, want)
	}
}
