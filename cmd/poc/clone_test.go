...
func TestBoot_ClonesHTTPSRepo(t *testing.T) {
	...
	sess, err := mgr.Create(context.Background(), "https://github.com/example/myrepo", "", "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestBoot_NormalizesSSHURL(t *testing.T) {
	...
	sess, err := mgr.Create(context.Background(), "git@github.com:example/myrepo.git", "", "")
	if err != nil {
		t.Fatal(err)
	}
	waitState(t, mgr, sess.ID, session.StateReady, 3*time.Second)
...
func TestBoot_CredentialsSetBeforeClone(t *testing.T) {
	...
	sess, _ := mgr.Create(context.Background(), "https://github.com/example/myrepo", "", "")
	waitState(t, mgr, sess.ID, session.StateReady, 3*time.Second)
...
func TestBoot_BranchClone(t *testing.T) {
	...
	sess, _ := mgr.Create(context.Background(), "https://github.com/example/myrepo", "feature-branch", "")
	waitState(t, mgr, sess.ID, session.StateReady, 3*time.Second)
