...
// JSONStore persists sessions and favorites as a JSON file protected by a
// mutex. Used for tests and local dev; production uses pgstore.Store.
...
func (s *JSONStore) Upsert(r Row) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *JSONStore) Get(id string) (Row, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.reload()
...
func (s *JSONStore) List() ([]Row, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.reload()
...
func (s *JSONStore) UpsertFavorite(f Favorite) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *JSONStore) DeleteFavorite(id string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *JSONStore) ListFavorites() ([]Favorite, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.reload()
